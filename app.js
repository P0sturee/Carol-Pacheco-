...
window.APP_BACKEND_ORIGIN = null; // ex: "https://pastry-api-core.lovable.app"
...
async function loadStoreStatus(){
  ...
  try{
    const { data, error } = await supabaseClient.rpc('get_store_status');
...
async function loadProducts(){
  ...
  try{
    ...
    grid.innerHTML = `<div class="empty-state">Não foi possível carregar o cardápio agora. Tente novamente em instantes.<br><small style="opacity:.6">${err.message || ''}</small></div>`;
  }
}
